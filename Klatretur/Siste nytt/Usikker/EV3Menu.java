...
public class EV3Menu {
	 ...
	 * This method takes care of the times we're gonna go deeper.
	 */
		...
		if (opt > 0) {
			if (gui.isUp(opt)) {
				if (!branchTree[currentMenu].browseUp()) {
					gui.setLight(EV3GUI.BLINK_RED, 400);
				}
			} else if (gui.isDown(opt)) {
	...
	private void doAction(byte opcode) {
		switch (opcode) {
			...
			case START_CLIMBING:
        		...
        		long millis;
				while (api.readButtons() == 0) {
	            	millis = System.currentTimeMillis() - startMillis;
		            if (!api.climbing) {
		                try {
		                    api.searchForStep(millis);
		                } catch (java.util.ConcurrentModificationException e) {
		                    //Do nothing and continue
		                }
		            } else {
		                if(!api.climbUpStep(api.kalvskinnet)) {
		                	break;
		                }
		            }
		        }
